...
public class ExplorerScene : MonoBehaviour {
	...
	void Start () {
		...
		transform.rigidbody.mass = 1f;
		transform.rigidbody.drag = Mathf.Infinity;
		transform.rigidbody.angularDrag = Mathf.Infinity;
		transform.rigidbody.useGravity = false;
		transform.rigidbody.isKinematic = false;
		transform.rigidbody.interpolation = RigidbodyInterpolation.None;
		transform.rigidbody.collisionDetectionMode = CollisionDetectionMode.Discrete;
		...
		transform.position = pointOfInterest;
	
		if (zFighting) {
			camera.nearClipPlane = maxHeight * 0.01f;
		}
		if (camera.farClipPlane < maxHeight * 8f) {
			camera.farClipPlane = maxHeight * 8f;
		}
		
		if (!speedChange){
			navigationSpeed = speedFactor / 7.5f;
		}
		
		fogDenseInitial = RenderSettings.fogDensity;
		
		if (darkerSun) {
			sunLightInitial = darkerSun.light.intensity;
		}
		
		logo = (Texture2D)Resources.Load("Logo");
		
		camView = camera.fieldOfView;
	}
	...
	void Update () {
		...
		if (transform.position.y < fogMinimumHeight) {	
			...
			if (darkerSun) {
				...
				darkerSun.light.intensity = sunLightInitial * ambientZone2;
				
				if (darkerSun.light.intensity < remainingLight) {
					darkerSun.light.intensity = remainingLight;
				}
			}
		}
        ...
        // Y Bounds	
        if (transform.position.y > (terrain.transform.position.y + terrainWidth) / 10f)
		    transform.position = new Vector3 (transform.position.x, (terrain.transform.position.y + terrainWidth) / 10f, transform.position.z);
        if (transform.position.y < terrain.transform.position.y + transform.gameObject.camera.nearClipPlane + 0.01f)
		    transform.position = new Vector3 (transform.position.x, terrain.transform.position.y + transform.gameObject.camera.nearClipPlane + 0.01f, transform.position.z);
		...
		if ((heightPos <= maxHeight * 0.3f)&&(heightPos >= maxHeight * 0.2f)) {
			...
			if (fOV) {
				camera.fieldOfView = Mathf.Lerp(camera.fieldOfView, camView, Time.deltaTime * fOVSpeed);
			}
			if (zFighting) {
				camera.nearClipPlane = maxHeight * 0.01f;
			}
		...
		if ((heightPos <= maxHeight * 0.2f)&&(heightPos >= maxHeight * 0.1f)) {
			...
			if (fOV) {
				camera.fieldOfView = Mathf.Lerp(camera.fieldOfView, camView / 1.1f, Time.deltaTime * fOVSpeed);
			}
			if (zFighting) {
				camera.nearClipPlane = maxHeight * 0.005f;
			}
		}
		if ((heightPos <= maxHeight * 0.1f)&&(heightPos >= maxHeight * 0.01f)) {
			...
			if (fOV) {
				camera.fieldOfView = Mathf.Lerp(camera.fieldOfView, camView / 1.1f / 1.1f, Time.deltaTime * fOVSpeed);
			}
			if (zFighting) {
				camera.nearClipPlane = maxHeight * 0.003f;
			}
		}
		if ((heightPos <= maxHeight * 0.01f)&&(heightPos >= maxHeight * 0.00275f)) {
			...
			if (fOV) {
				camera.fieldOfView = Mathf.Lerp(camera.fieldOfView, camView / 1.1f / 1.1f / 1.1f, Time.deltaTime * fOVSpeed);
			}
			if (zFighting) {
				camera.nearClipPlane = maxHeight * 0.002f;
			}
		}
		if ((heightPos <= maxHeight * 0.00275f)&&(heightPos >= 0f)) {
			...
			if (fOV) {
				camera.fieldOfView = Mathf.Lerp(camera.fieldOfView, camView / 1.1f / 1.1f / 1.1f / 1.1f, Time.deltaTime * fOVSpeed);
			}
			if (zFighting) {
				camera.nearClipPlane = 0.225f;
			}
		}
